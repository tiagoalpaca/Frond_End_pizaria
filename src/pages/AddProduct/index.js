...
const AddProduct = () => {
    ...
    const handleSubmit = async (event) =>{
        ...
        const categoriesId = selected.map(caterory => {
            return{
                _id: caterory.value
            }
        })
    ...
    <section className='my-12 max-w-screen-xl mx-auto px-6'>
        ...
        <form onSubmit={handleSubmit} className='grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-10 mt-6'>
            <div className='flex flex-col space-y-2'>
                ...
                <textarea name="descricao" id="descricao" cols="30" rows="5" required
                onChange={handleChangeValues}
            ...
            <div className='flex flex-col space-y-2'>
                ...
                <input type="text" id='preco' name='precoUnitario' required
                onChange={handleChangeValues}
                className='w-full px-4 py-3 rounded-lg ring-red-200 border border-gray-300 focus:ring-4 focus:outline-none transition duration-300 focus: shadow-xl' >
                </input>
                ...
                <div>
                    <button className="w-full px-2 py-3 bg-primary text-white  ring-red-200 border border-gray-300 focus:ring-4 focus:outline-none transition duration-300 focus: shadow-xl">Adicionar</button>
                </div>
            </div>
        </form>
    </section>
